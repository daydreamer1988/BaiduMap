 ...
 *
 *         (不推荐)或在状态更新时，让InfoWindow消失，状态结束后再显示。这样的话布局不会根据内容而变化，如果背景是透明的，还是会叠在一起。
 *         状态结束后再显示才会显示正常
 *
 */
        ...
        mBaiduMap.setOnMarkerClickListener(new BaiduMap.OnMarkerClickListener() {
            ...
            public boolean onMarkerClick(Marker marker) {
                ...
                mMapView.postDelayed(new Runnable() {
                    ...
                    public void run() {
                        tv.setText("fuck");
                        mBaiduMap.animateMapStatus(MapStatusUpdateFactory.newMapStatus(new MapStatus.Builder().overlook(-1f).build()), 100);
                    }
                }, 1000);
